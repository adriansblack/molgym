...
class SphericalUniform(SphericalDistribution):
    def __init__(self, batch_shape=torch.Size(), device=None, dtype=torch.float) -> None:
        super().__init__(batch_shape, validate_args=False, device=device, dtype=dtype)
...
class SO3Distribution(SphericalDistribution):
    def __init__(
        ...
        super().__init__(batch_shape=torch.Size(a_lms.shape[:-1]), validate_args=False, device=device, dtype=dtype)

        self.lmax = lmax
        self.b_lms = self.normalize(a_lms)
        self.gamma = gamma

        self.spherical_uniform = SphericalUniform(batch_shape=self.batch_shape, device=device, dtype=dtype)
        self.uniform_dist = torch.distributions.Uniform(low=0.0, high=1.0, validate_args=False)
        self.log_z = self.compute_log_z()

    def __str__(self) -> str:
        return f'SO3Distribution(l_max={self.lmax})'
    ...
    def log_prob_unnormalized(
        ...
        # b_lms: [B, (lmax + 1)^2]
        # y_lms: [S, B, (lmax + 1)^2]
        f = torch.square(torch.sum(self.b_lms * y_lms, dim=-1))
        return -self.gamma * f

    def compute_log_z(self) -> torch.Tensor:
        grid = quadpy.u3._lebedev.lebedev_071()  # pylint: disable=protected-access
        grid_points = torch.tensor(grid.points.transpose(), dtype=self.dtype,
                                   device=self.device).unsqueeze(1)  # grid_points: [S, 1, 3]
        log_probs_unnormalized = self.log_prob_unnormalized(grid_points)  # [S, B]
        weights = torch.tensor(grid.weights, dtype=self.dtype, device=self.device).unsqueeze(-1)  # [S, 1]
