...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--min_epoch', help='minimum epoch', default=0, type=int, required=False)
    parser.add_argument('--max_epoch', help='maximum epoch', default=sys.maxsize, type=int, required=False)
    return parser.parse_args()


def plot(data: pd.DataFrame, min_epoch: int, max_epoch: int, output_path: str) -> None:
    data = data[(min_epoch < data['epoch']) & (data['epoch'] < max_epoch)]
    ...
    ax.plot(valid_data['epoch'], valid_data['loss']['mean'], zorder=1, label='Validation')
    ax.fill_between(
        x=valid_data['epoch'],
    ...
    ax.plot(train_data['epoch'], train_data['loss', 'mean'], zorder=1, label='Training')
    ax.fill_between(
        x=train_data['epoch'],
...
def main():
    ...
    for name, group in data.groupby(['name']):
        plot(group, min_epoch=args.min_epoch, max_epoch=args.max_epoch, output_path=f'{name}.pdf')
